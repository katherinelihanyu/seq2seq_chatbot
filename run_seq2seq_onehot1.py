...
def process_row(row, prev_row):
    vec_einput = np.zeros((max_sentence_length, num_unique_words),dtype=np.float16)
    ...
    vec_dinput = np.zeros((max_sentence_length, num_unique_words),dtype=np.float16)
    vec_doutput = np.zeros((max_sentence_length, num_unique_words),dtype=np.float16)
    words = re.findall(pattern, row["Text"])
    words = [word.lower() for word in words if word.lower() in VOCAB]
    if len(words) == 0:
        return None
